...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    @IBAction func websiteMenuItemClicked(_ sender: Any) {
        if let url = URL(string: "http://lab.rayps.com/ses"), NSWorkspace.shared.open(url) {
        }
    }
    ...
    @IBAction func sourceCodeMenuItemClicked(_ sender: Any) {
        if let url = URL(string: "https://github.com/RayPS/Search-Engine-Switcher/"), NSWorkspace.shared.open(url) {
        }
    }
